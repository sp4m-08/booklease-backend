    ...
    <div className={`flex flex-wrap gap-1 items-center ${className}`}>
      {slots.map((s, i) => {
        ...
        return (
          <span
            key={i}
            className={`relative border-2 border-black ${isRented ? 'bg-gray-300 text-gray-500' : bgClass} text-black px-2 py-0.5 text-xs font-black shadow-xs inline-flex items-center overflow-hidden`}
            title={isRented ? "Currently Rented Out" : "Available"}
          >
            {isRented && (
              <span className="absolute inset-0 flex items-center justify-center">
                <span className="w-full h-[4px] bg-black absolute z-10"></span>
              </span>
            )}
            <span className={isRented ? "opacity-40" : ""}>{s}</span>
          </span>
        );
      })}
    </div>
